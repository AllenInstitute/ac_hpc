#!/bin/bash
#SBATCH --job-name=segment_cutout
#SBATCH --ntasks=1
#SBATCH --nodes=1
...
bounds='/allen/programs/celltypes/workgroups/em-connectomics/laughla/Slurm/Segment/cutout/bounds.txt'
weights_file='/allen/programs/celltypes/workgroups/em-connectomics/laughla/Slurm/Segment/single/best.ckpt'
config_file='/allen/programs/celltypes/workgroups/em-connectomics/laughla/Slurm/Segment/cutout/slurm.config'
#cloud_params="{'AWS_key':'' , 'AWS_sec_key':'', 'region':'us-east-1', 'bucket':'ac-test', 'endpoint':'http://aidc-ceph1-prd:8000'}"

in_files="/allen/programs/celltypes/workgroups/em-connectomics/laughla/Slurm/Segment/cutout/in_files.txt"
...
NXF_MAIN="/allen/programs/celltypes/workgroups/em-connectomics/laughla/Slurm/Segment/cutout/main.nf"

module load java/jdk-21.0.3

"$NXF_EX" run "$NXF_MAIN" --in_files "$in_files" --out_dir "$out_dir" --bounds "$bounds" --mask_dir "$mask_dir" --weights_file "$weights_file" --cloud_params "$cloud_params" -c "$config_file" -profile hpc
