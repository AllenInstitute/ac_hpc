...
class SegmentZarrModule(argschema.ArgSchemaParser):
    ...
    def run(self):
        ...
        input_arr = open_tensor(os.path.join(self.args['input_zarr'],'0'), bytes_limit= 100_000_000, driver='zarr')
        ...
        print(rescale, input_arr.kvstore.path,output_arr.kvstore.path)
        
        if self.args["bound_box"]:
            segment_gunpowder(input_arr, output_arr, self.args["weights_file"], iter_size=(64,64,64), batch_size=10, cutout=self.args["bound_box"], cpus=10, preprocess=preprocess, mask_file=self.args['mask_path'], dsfactor=self.args['dsfactor'], gpu_device=0)
        else:
            segment_gunpowder(input_arr, output_arr, self.args["weights_file"], iter_size=(64,64,64), batch_size=10, cutout=None, cpus=10, preprocess=preprocess, mask_file=self.args['mask_path'], dsfactor=self.args['dsfactor'], gpu_device=0)
