...
###merge pairs 
def merge_pairs_direct(skels, pair_data, min_nodes=5):
    merge_num = 0
    ...
    neuro_ids = np.array([str(x) for x in skels.id])
    remove = [] 
    ...
    for ind,com in enumerate(cc):
        ...
        for neu in com:
            ...
            if len(s_ind[0])>0:
                ...
                if skels[s_ind].n_nodes >= min_nodes:
                    group.append(skels[s_ind])
...
class SkeletonizeProbabilitiesModule(argschema.ArgSchemaParser):
    ...
    def run(self):
        ...
        skels = TS_skeletonize_volume(array, chunk_size=[100,100,100], n_jobs=self.args["n_jobs"], prob_thresh=.05, label_size_threshold=self.args["label_size_threshold"], overlap=4, cutout=self.args["bound_box"])
        print("Completed skeletonization")
        
        match_pairs, rem_skels = find_remove_node_overlap(array.shape,skels, overlap=4, block_size=[500,500,500], n_jobs=self.args["n_jobs"])
        print("Completed matching")
        
        nonmerge,merge,remap = merge_pairs_direct(rem_skels, match_pairs)
        print("Completed reconnection")
        
        out_skels = navis.NeuronList([nonmerge,merge])
        write_navis_skels_tar(skels_outpath,merge)
