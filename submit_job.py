...
local_template = """ "{nextflow_exec}" run "{nextflow_main}" {method_parameters} -c "{config_file}" -profile standard """
...
def normalize_out_dirs(d):
    """Recursively remove trailing slashes from any *_out_dir keys in the dict."""
    for key, value in d.items():
        if isinstance(value, dict):
            normalize_out_dirs(value)
        elif key.endswith("_out_dir") and isinstance(value, str) and value:
            d[key] = value.rstrip("/")
...
required_params_map = {
    ...
    "equalize": ["equal_out_dir"],
    "deskew_zarr": ["deskew_out_dir"],
    "deskew_tiff": ["deskew_out_dir"]    
}
...
class SegmentParameters(argschema.ArgSchema):
    ...
    dsfactor = argschema.fields.Int(required=False, dump_default=16, allow_none=True)
    mip = argschema.fields.Int(required=False, dump_default=0, allow_none=True)
...
class Methods(ArgSchema):
    ...
    run_local = argschema.fields.Boolean(required=True, dump_default=False)
    equalize = Nested(EqualizeParameters, required=False)
    deskew_zarr = Nested(DeskewZarrParameters, required=False)
    deskew_tiff = Nested(DeskewTiffParameters, required=False)
    ...
    cloud_params = Nested(CloudOptions, required=True, dump_default={'AWS_key':'None' , 'AWS_sec_key':'None', 'region':'None', 'endpoint':'None'})
    nf_files = Nested(
        NextflowFiles,
        required=True,
        dump_default={
...
class SubmitJobModule:
    ...
    def run(self):
        ...
        all_parameters = format_params_for_nextflow(method_parameters) + input_params + cloud_params
        slurm_out = self.args.get("slurm", {}).get("out_dir") or str(REPO_DIR / "slurm_logs")
            
        if self.args['run_local']:
            job_script = local_template.format(
        ...
        #slurm logs
        slurm_dir = Path(REPO_DIR) / "slurm_scripts"
        slurm_dir.mkdir(exist_ok=True)
        filename = f"{method}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sh"
        file_path = slurm_dir / filename
        with open(file_path, "w") as f:
            f.write(job_script)
        print(f"Saved job script to {file_path}")
        
        if self.args['hpc_api_url']:                                                                   
            try:
    
                #payload = {'script': job_script}
                #response = requests.post(self.args['hpc_api_url'], json=payload)
...
if __name__ == "__main__":
    ...
    if 'deskew' in method:
        check_required_params(method, args, required_params_map)
        if 'deskew_zarr' in args and args['deskew_zarr']:
            args['deskew_zarr']['deskew_options'] = '"' + args['deskew_zarr']['deskew_options'] + '"'
    elif method == "segment":
        ...
        check_required_params('equalize', args, required_params_map)
